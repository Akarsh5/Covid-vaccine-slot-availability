...
6. If you're unable to receive emails, go to gmail account > manage your account option > Security > Allow less secured apps, turn it on. (It might be off by default, turn off 2FA authentication if you're unable to see less secured section).
7. Double click covid.py file to run the code ( this might not run infinitely so use the other method to run the code ->) OR open/edit it in Python IDLE (after doing above steps) and press F5 to run module. Type the age, pincode, email addresses of sender and receiver (you can press enter to keep same if you want to send yourself), password of gmail account (password will be hidden so make sure you type it correctly). 
